...
(defun ht-delete (key ht)
  (let* ((size (length (ht-array ht)))
    ...
    (do* ((count 0 (1+ count))
         ...
         ((or (null item) (= count size)))
      (when (funcall (ht-comp ht) key (car item))
        (setf (aref (ht-array ht) i) (list "DELETED" nil))
        (decf (ht-count ht))
        ))
    )
